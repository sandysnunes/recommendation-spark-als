...
// spark-submit --class RecomendacaoPecasClientesSemClusters --master local[*] --executor-memory 1g --driver-memory 15g --conf "spark.executor.extraJavaOptions=-verbose:gc -XX:+UseSerialGC -XX:+UseCompressedOops -XX:+UseCompressedStrings  -XX:+PrintGCDetails -XX:+PrintGCTimeStamps -XX:PermSize=256M -XX:MaxPermSize=512M" recommendation-spark-als-1.0-SNAPSHOT.jar   2018.csv
object RecomendacaoPecasClientesSemClusters extends App {
  ...
  val conf: SparkConf = new SparkConf().setAppName("RecomendacaoPecasClientesSemClusters")
  ...
  val clientesBuckets = rawRatings.groupBy(_.user) //agrupa ratings por cliente
